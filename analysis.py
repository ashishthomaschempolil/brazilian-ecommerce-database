...
sellers["city_state"] = sellers["seller_city"] + "_" + sellers["seller_state"]
geolocation["city_state"] = (
    geolocation["geolocation_city"] + "_" + geolocation["geolocation_state"]
)

# get city states values not in geolocation
city_states = [
    city_state
    for city_state in sellers["city_state"].unique()
    if city_state not in geolocation["city_state"].unique()
]
...
for index, row in geolocation.iterrows():
    if row["geolocation_city"] not in cities:
        row["geolocation_city"] = difflib.get_close_matches(
            row["geolocation_city"], cities, n=1, cutoff=0.8
        )
...
for index, row in sellers.iterrows():
    if row["seller_city"] not in cities:
        row["seller_city"] = difflib.get_close_matches(
            row["seller_city"], cities, n=1, cutoff=0.8
        )
...
# %%
geolocation.drop(
    "geolocation_zip_code_prefix", axis=1, inplace=True
)  # drop the zip code prefix column
...
geolocation_dict = dict(
    zip(geolocation["geolocation_city"], geolocation["geolocation_id"])
)

sellers["geolocation_id"] = sellers["seller_city"].map(
    geolocation_dict
)  # .astype('int64')
...
geolocation.geolocation_zip_code_prefix.value_counts()[
    geolocation.geolocation_zip_code_prefix.value_counts().index == 1046
]
...
for index, row in sellers.iterrows():
    if (
        ...
        not in geolocation.geolocation_zip_code_prefix.values
    ):
        print(row["seller_zip_code_prefix"])
        geolocation = geolocation.append(
            {
                "geolocation_zip_code_prefix": row["seller_zip_code_prefix"],
